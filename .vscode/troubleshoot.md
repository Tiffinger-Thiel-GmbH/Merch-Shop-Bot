...
## npm Install Failed with `EISDIR` or Cleanup Errors

### Symptoms

`atk deploy` fails during `cli/runNpmCommand` with errors such as:
...
Deprecated-package warnings, such as `uuid@8` or `uuid@10`, are warnings and are not the immediate failure.

### Cause

The existing `node_modules` tree is corrupted, partially installed, locked by another process, or was created using a different OS/runtime. Windows-style paths in logs from a Linux VS Code Server session indicate that Node/npm resolution should also be checked.
...
If the output references the wrong operating system or runtime, fix the Node/npm installation before continuing.

Quarantine dependencies and reinstall:

```bash
mv node_modules "node_modules.broken.$(date +%s)" 2>/dev/null || true
...
If the same cleanup error persists:

```bash
npm cache verify
npm cache clean --force
mv node_modules "node_modules.broken.retry.$(date +%s)" 2>/dev/null || true
npm ci 2>/dev/null || npm install
```

Then verify the project before retrying ATK:
...
Do not remove `package-lock.json` unless it is known to be corrupt. Close other Node/npm processes that may be locking files. If the failure continues, inspect the full npm debug log referenced after `npm error A complete log...`.

### Deprecation Warnings

`npm warn deprecated` messages do not normally fail installation. Upgrade affected direct dependencies separately after restoring a successful install, and validate compatibility before changing transitive dependencies.
